...
func TestParseServeOptionsRequiresSafeComposition(t *testing.T) {
	t.Parallel()
	image := "registry.invalid/oberth-ci@sha256:" + strings.Repeat("a", 64)
	options, err := parseServeOptions([]string{"--runner-image", image})
	if err != nil {
		t.Fatal(err)
	}
	if options.runnerImage != image || options.maxConcurrent != 3 || options.jobTimeout != 12*time.Hour ||
		options.auditTSAURL != "https://timestamp.sectigo.com/rfc3161" || options.auditRekorURL != "https://rekor.sigstore.dev" ||
		options.auditAnchorInterval != 10*time.Minute || options.auditAnchorMaxAge != 30*time.Minute {
		t.Fatalf("options = %+v", options)
	}
	for _, arguments := range [][]string{
		{"--runner-image", "registry.invalid/oberth-ci:latest"},
		{"--runner-image", image, "--database", "/tmp/outside.db"},
		{"--runner-image", image, "--ssh-listen", ":8443", "--https-listen", ":8443"},
		{"--runner-image", image, "--max-concurrent-jobs", "0"},
		{"--runner-image", image, "--audit-tsa-url", "file:///tmp/fake-tsa"},
		{"--runner-image", image, "--audit-tsa-url", "http://timestamp.example.test"},
		{"--runner-image", image, "--audit-tsa-url", "https:///missing-host"},
		{"--runner-image", image, "--audit-tsa-url", "HTTPS://timestamp.example.test"},
		{"--runner-image", image, "--audit-tsa-url", "https://user:pass@timestamp.example.test"},
		{"--runner-image", image, "--audit-tsa-url", "https://timestamp.example.test/path#fragment"},
		{"--runner-image", image, "--audit-tsa-url", "https://timestamp.example.test/path#"},
		{"--runner-image", image, "--audit-rekor-url", "http://rekor.example.test"},
		{"--runner-image", image, "--audit-rekor-url", "file:///tmp/fake-rekor"},
		{"--runner-image", image, "--audit-rekor-url", "HTTPS://rekor.example.test"},
		{"--runner-image", image, "--audit-rekor-url", "https://rekor.example.test/path#fragment"},
		{"--runner-image", image, "--audit-rekor-url", "https://rekor.example.test/path#"},
		{"--runner-image", image, "--audit-anchor-interval", "1h", "--audit-anchor-max-age", "10m"},
		{"--runner-image", image, "--audit-tsa-roots", "relative.pem"},
	} {
...
func TestAdministrativeBootstrapRegistersUpstreamAndBoundToken(t *testing.T) {
	...
	if !strings.Contains(upstreamOutput.String(), "registered upstream codeberg") {
		t.Fatalf("upstream output = %q", upstreamOutput.String())
	}
	...
	if len(lines) != 3 || !strings.HasPrefix(lines[0], "TLS certificate fingerprint: SHA256:") || !strings.HasPrefix(lines[2], "oberth_") {
	...
	authenticated, err := (auth.Authenticator{Tokens: database}).Authenticate(ctx, lines[2])
	if err != nil {
		t.Fatal(err)
	}

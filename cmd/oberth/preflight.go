...
func runPreflight(ctx context.Context, arguments []string, output io.Writer) error {
	...
	switch arguments[0] {
	case "drift":
		return runPreflightDrift(ctx, arguments[1:], output, installer.DefaultRunHelm, installer.ResolveHelmKubeArgs)
	default:
...
// kubeResolver resolves kubeconfig to helm arguments and a target description.
// The production implementation is installer.ResolveHelmKubeArgs.
type kubeResolver func(contextName string) (helmArgs []string, target string, err error)
...
func runPreflightDrift(ctx context.Context, arguments []string, output io.Writer, runHelm helmRunner, resolveKube kubeResolver) error {
	...
	kubeContext := flags.String("context", "", "kubeconfig context (default: same resolution as the oberth binary)")
	...
	if resolveKube == nil {
		resolveKube = installer.ResolveHelmKubeArgs
	}

	// Resolve kubeconfig to the same target the rest of the binary uses.
	// When --context is given, this returns --kube-context; when not, it
	// mirrors LoadKubeConfigForContext's k3s fallback chain and returns
	// --kubeconfig when the k3s path was selected.
	kubeArgs, target, err := resolveKube(*kubeContext)
	if err != nil {
		return fmt.Errorf("resolve kubeconfig for helm: %w; pass --context explicitly or set KUBECONFIG", err)
	}

	// Step 1: Retrieve the live release's merged values.
	getValuesArgs := []string{"get", "values", *release, "-n", *namespace, "-o", "yaml"}
	getValuesArgs = append(getValuesArgs, kubeArgs...)
	valuesOut, err := runHelm(ctx, getValuesArgs)
	if err != nil {
		return fmt.Errorf("helm get values failed for release %q in namespace %q (%s): %w; "+
			"if the cluster target is wrong, pass --context or set KUBECONFIG",
			*release, *namespace, target, err)
	}
	...
	if trimmed == "" {
		return fmt.Errorf("live values document is empty for release %q in namespace %q (%s); refusing to template against chart defaults", *release, *namespace, target)
	}
	...
	templateArgs = append(templateArgs, kubeArgs...)
	_, templateErr := runHelm(ctx, templateArgs)
	if templateErr != nil {
		return fmt.Errorf("chart template failed against live values (%s) — a release upgrade would fail the same way: %w", target, templateErr)
	}

	_, _ = fmt.Fprintf(output, "preflight drift: OK — chart %s templates cleanly against live values from release %s/%s (%s)\n", *chart, *namespace, *release, target)
	return nil
}
